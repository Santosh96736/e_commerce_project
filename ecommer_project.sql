...
SELECT COUNT(*) AS total_cancel FROM orders WHERE order_status = "canceled";

-- AVERAGE DAY TAKEN TO DELIVARY A ORDER

SELECT ROUND(AVG(DATEDIFF(order_delivered_customer_date,order_purchase_timestamp)), 2) AS avg_day
FROM orders
...
SELECT CONCAT(ROUND((COUNT(*) / (SELECT COUNT(*) FROM orders)) * 100, 2),"%") AS late_deliveries
FROM orders
WHERE order_delivered_customer_date > order_estimated_delivery_date;

-- COUNT HOW MANY CUSTOMER GIVES MORE THAN 3 STAR

SELECT COUNT(*) AS total_review FROM order_reviews
...
-- 2017 orders vs. 2018 orders 

WITH order_data AS (SELECT YEAR(order_purchase_timestamp) AS year, COUNT(*) AS total_orders
FROM orders
GROUP BY year)
